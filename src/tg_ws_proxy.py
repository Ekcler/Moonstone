...
try:
    from src.proxy.utils import *
    from src.proxy.stats import stats
    from src.proxy.config import proxy_config, parse_dc_ip_list, start_cfproxy_domain_refresh, CFPROXY_DEFAULT_DOMAINS
...
async def _run(stop_event: Optional[asyncio.Event] = None):
    ...
    log.info(f"[DEBUG] _run: using port={proxy_config.port}, host={proxy_config.host}")
    try:
        server = await asyncio.start_server(client_cb, proxy_config.host, proxy_config.port)
        log.info(f"[DEBUG] _run: server started on {proxy_config.host}:{proxy_config.port}")
    except Exception as e:
        log.error(f"[DEBUG] _run: failed to start server: {e}")
        return
    ...
    try:
        async with server:
            if stop_event:
                log.info(f"[DEBUG] stop_event present, setting up wait task")
                ...
                if stop_task in done:
                    log.info(f"[DEBUG] stop_event triggered, closing server")
                    server.close()
                    await server.wait_closed()
                    # Cancel all pending client tasks
                    for task in list(_client_tasks):
                        if not task.done():
                            task.cancel()
                    _client_tasks.clear()
                    ...
                    log.info(f"[DEBUG] serve_forever ended, cancelling stop_task")
                    stop_task.cancel()
                    try:
                        await stop_task
                    except asyncio.CancelledError:
